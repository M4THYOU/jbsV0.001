            ...
            with open(schedule_path) as json_file:
                ...
                for day_of_week, shifts_dict in user_working_indexes_dict.items():
                    for email, hours_index_list in shifts_dict.items():
                        ...
                        writer.writerow([
                            email, day_of_week, time_24_hr, time_12_hr, schedule, start_date, end_date
                        ])
...
def shift_hours_sorter(shift_list):
    """
    Sorts a list of integers so that the first occuring int in the list is the first hour of the shift.
    Solves the overnight shift problem. An overnight shift will generate a list like [0, 1, 22, 23].
    However, the start int is 22, not 0. This functions sorts the given list to [22, 23, 0, 1].
    :param shift_list: List of ints for the hours a user is working. Ex. [12, 13, 14] or [0, 1, 22, 23]
    :return: Resorted list of ints, starting with the beginning of the shift. Ex. [12, 13, 14] or [22, 23, 0, 1]
    """
    new_list = []
