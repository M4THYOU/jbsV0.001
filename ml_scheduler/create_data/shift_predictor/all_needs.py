...
def needs_from_past_schedules(path_to_schedules):
    ...
    for day_name, user_list in raw_needs_dict.items():
        for hours_list in user_list:
            ...
            avg_processed_needs_dict['needs'][day_name].append(rounded_average_needs)
            med_processed_needs_dict['needs'][day_name].append(median_needs)

    return avg_processed_needs_dict, med_processed_needs_dict


def get_needs_methods(path_to_schedules, company, department):
    """
    :param path_to_schedules: String path to directory containing the JSON schedule files.
    :param company: String name of the company.
    :param department: String name of a department within the specified company.
    :return: Tuple. (manual_needs, auto_avg_needs, auto_median_needs). They are each dicts in a list for consistency
    when using them in create_data.
    """
    manual_needs = get_full_needs(company, department)
...
def compare_needs_methods(path_to_schedules, company, department):
    """
    DEPRECATED. Rework when needed.
    ...
    """
    manual_needs, auto_avg_needs, auto_median_needs = get_needs_methods(path_to_schedules, company, department)
    manual_day_needs = manual_needs['needs']
    auto_avg_needs = auto_avg_needs['needs']
    auto_median_needs = auto_median_needs['needs']
